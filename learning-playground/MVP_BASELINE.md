...
- **Kennedi's Story Stage** (slice 1 of 7, `#story-stage` — direct route + Parent Panel launch; deliberately NOT on the home grid yet): the first storytelling runtime. One fixed, fully narrated tale ("Poppy and Biscuit's Forest Adventure", a Lost Friend story where Biscuit is explicitly safe from the first mention) with a six-scene play path, two real decisions (WHERE to look — two distinct illustrated consequences — and HOW to help — two ending variants), a one-line readable caption per scene (usable when speech is off), a neutral six-dot story path, no autoplay (the child advances by choice or Continue), Repeat, and Tell It Again. **Architecturally non-evaluative**: Story Stage is its own route, not a catalog activity — the runtime takes speech only and has no event sink, so creative choices can never become skill evidence, coverage producers, or transfer contexts. Since slice 3 the story opens with the **Pick Three setup**: three focused selection steps (Who is the story about? / Where does it happen? / What happens?) with large illustrated cards — tapping a card speaks its authored line and shows a selected state without auto-advancing; a large Next control (enabled only after a pick) moves forward; a visual summary of the three picks and one large Start control launches the fully narrated story. Steps only ever offer entities some story family supports given the picks so far (pure selection helpers make unsupported combinations impossible by construction), and endings gained a New story control back to setup. Slice 7 completes the arc: the setup screen carries a story-room curtain valance (decorative, aria-hidden, §23-restrained), endings warm the stage frame with a static golden glow, the carried art debts are paid (the owl's face, Biscuit's gripping peek, Luna's flowing mane), story-stage hover motion goes still under prefers-reduced-motion, and the child-entry decision is written up in `docs/proposals/story-stage-child-entry.md` (recommendation: featured wide home card; NOT implemented — awaiting owner approval, home untouched). Since slice 6 stories leave a **non-evaluative local record** (spec-scoped fields only: session id, mode, family, the three picks, choice path, ending id, timestamps, completed/left-early — never narration audio, interpretations, or scores), written by a narrow history sink that is deliberately not the attempt-event log; records persist locally, ride along in the parent export as `story_history`, are removed by Clear Progress Data, and surface in a parent-readable **Recent Stories** section ('Milo the Puppy at The Cloud Village — A Special Delivery · Tell It Together · Completed'). No correctness, no mastery claims, no trait inference — creative choices are narrative preferences. Since slice 5 the family owns the narration mode: a parent setting (`story_mode`: Tell Me a Story | Tell It Together, default narrated, changed in a Parent Panel section — the child never chooses) selects between full narration and **Tell It Together**, where scenes stay quiet and a compact collapsible storyteller-cue panel (what happens / keep true / a question to ask / optional silly twist) sits between the scene and the child controls, with a "Play the story line" fallback that speaks and reveals the authored narration so the session is never stranded; the active mode is visible on the launch card and the setup screen. Since slice 4 the shelves hold the real content pack: **three authored story families** — The Lost Friend, The Broken Thing (a quiet music box, fixed without blame), and The Special Delivery (sunshine berries to the surprise party, distinct from Bear Cafe) — over five original characters (Poppy, Finn the dragon, Robo, Luna the unicorn, Milo the puppy) and four settings (enchanted forest, moon castle, cozy town, cloud village), 38 supported combinations in all. Scene art is compositional (setting backdrop × selected hero in four moods × family beat props), compatibility lists keep every combination coherent (Milo never hunts a puppy friend; the friend tokens exist only on the friend problem), and the validator dry-resolves every supported combination so an incoherent pairing cannot ship. Since slice 2 the tale is authored data, not hardcoded scenes: a typed story pack (characters / settings / problems / families whose narration is a template over a closed token set — controlled substitution, never mad-libs) resolved deterministically by a pure resolver, with every scene already carrying its adult storyteller cue so Tell It Together (slice 5) is a runtime feature over existing data, and a pure pack validator (duplicate ids, broken references, unreachable scenes, dead ends, cycles, fake choices, path bounds, unknown tokens, missing cues, external URLs) proven rule-by-rule with mutation tests; the resolved narration is test-pinned byte-identical to slice 1, so the child experience did not move. Remaining slices (Pick Three setup, three story families, Tell It Together surfaces, non-evaluative story history, story-room polish + child-entry proposal) are documented in `docs/contracts/story-stage.contract.md`.
- **Kennedi's Dress-Up Studio** (`#dress-up` — Parent Panel launch; deliberately NOT on the home grid): a creative-play dress-up game built on the Story Stage integration pattern (its own route, no event sink, so nothing is scored). The child dresses an original doll, **Luna** — three skin tones, four hairstyles, three hair colors, optional glasses — from a free-play wardrobe (tops, bottoms, dresses that replace top+bottom, shoes, jackets, multi-select accessories), places her in one of four scenes (birthday, park, dance stage, rainy day), and finishes the look into a saved **fashion card** (scene + doll + chosen frame + optional sticker) with a one-shot reveal. Every choice is expressive and unscored; tapping a selected item removes it. The finished card stays until a child-controlled action (Play Again / Home), and earlier looks live in a bounded revisit shelf reachable without earning access again. Preservation is a non-evaluative `FashionCardCompletion` (the ownership completion object — the exact doll/outfit/scene/frame/sticker), stored via a narrow history sink that is deliberately not the attempt-event log; records are capped, ride along in the parent export as `fashion_card_history`, and are removed by Clear Progress Data. All art is original inline SVG (Luna doll + wardrobe + scenes + frames + stickers, Category A, look-approval pending owner sign-off); a child-home entry (an Art picker within the four-card limit) is a documented follow-up.
