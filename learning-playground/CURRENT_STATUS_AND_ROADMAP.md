...
- Video Vault shell for validated repo-bundled, parent-approved local videos,
  reachable by direct route and intentionally empty until content is added.
...
- Add the first approved repo-bundled local clip only with a separate
  evidence-bearing vocabulary response activity.
