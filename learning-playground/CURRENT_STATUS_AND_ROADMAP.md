...
- Video Vault with one validated repo-bundled, parent-approved local narrated
  clip and a manual handoff to a separate three-picture vocabulary response.
...
- Catalog-derived difficulty coverage for every curriculum rung, including the
  exact current gap inventory and parent-only current-rung explanation.
...
- Fill one reported difficulty gap with an approved existing-game variant,
  starting with counting difficulty 2-3; do not generate broad content.
