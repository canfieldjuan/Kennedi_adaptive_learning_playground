...
const checks = [
  ...
  },
  {
    ...
    requiredText: [
      ...
      'publisher_pid: process.pid', 'capacityOrphan', 'process.kill(pid, 0)',
      'symlinkSync(ownerId, lockPath)', 'readlinkSync(lockPath)', 'reclaimPublication',
      'prepareCapacityRelease', 'finishCapacityRelease', 'findOwnedCapacityRelease',
  ...
  },
  {
    ...
    requiredText: [
      ...
      'an old completion retry ignores a newer active wake on the same head',
      'an exact receipt wins over a newer shared transition marker',
      ...
      'abandon removes active publication residue before releasing its slot',
      'abandon finishes its quarantined slot after marker cleanup',
      'old abandon cleanup never touches a newer active owner with a reused token',
      'recovers dead capacity publication ownership',
      'recovers dead transition publication ownership',
      'does not steal live transition publication ownership',
      'fails closed on malformed publication ownership',
      'recovers dead completed-receipt publication ownership',
      ...
      'an abandon transition remains blocking after active removal while its slot exists',
      ...
      'fails closed at the wake-record capacity',
      'reclaims dead-publisher reservations that have no active owner',
      'reclaims a dead publisher active temporary file with its reservation',
      'two real processes cannot both acquire the same PR/head',
      'a burst of real acquire processes returns ownership decisions without races',
      ...
      'a burst of real completion retries never loses both receipt and active state',
