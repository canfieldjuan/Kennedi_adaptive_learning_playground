/**
 * Generic tap-choice activity runtime for early MVP activities.
 * Covers prompt, repeat, tap feedback, hint, completion, and local events.
 */
...
export function renderTapChoiceActivity(
  parent: HTMLElement,
  options: TapChoiceOptions
): void {
...
export function destroyTapChoiceActivity(): void {
...
function getChoices(activity: LearningActivity): TapChoiceChoice[] {
  const choices = activity.content.choices;
  if (!Array.isArray(choices)) return [];

  return choices.filter((choice): choice is TapChoiceChoice => {
...
function getCorrectChoice(
  activity: LearningActivity,
  choices: TapChoiceChoice[]
): TapChoiceChoice | undefined {
...
function speakAndPlay(options: TapChoiceOptions, feedback: FeedbackRule): void {
  if (feedback.speech) {
...
function createAttemptEvent(params: {
  ...
  promptText: string;
  selectedChoice: TapChoiceChoice;
  correctChoice: TapChoiceChoice;
